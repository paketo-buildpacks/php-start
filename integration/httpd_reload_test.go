	...
	context("when BP_LIVE_RELOAD_ENABLED is true", func() {
		...
		it("will reload httpd config files", func() {
			...
			Expect(logs).To(ContainLines(
				...
				MatchRegexp(`    HTTPD: watchexec --watch /workspace/\.httpd\.conf\.d --on-busy-update signal --signal SIGHUP -- httpd -f /layers/.*/php-httpd-config/httpd\.conf -k start -DFOREGROUND`),
