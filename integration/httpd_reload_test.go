...
func testHttpdReload(t *testing.T, context spec.G, it spec.S) {
	...
	context("when BP_LIVE_RELOAD_ENABLED is true", func() {
		...
		it("will reload httpd config files", func() {
			...
			err = docker.Container.Exec.WithUser("0:0").ExecuteBash(container.ID, "sed 's/original-httpd-value/reloaded-httpd-value/g' /workspace/.httpd.conf.d/header-server.conf > /tmp/sed_tmp && cat /tmp/sed_tmp > /workspace/.httpd.conf.d/header-server.conf")
			Expect(err).NotTo(HaveOccurred())
		...
		it("will reload FPM config files", func() {
			...
			err = docker.Container.Exec.WithUser("0:0").ExecuteBash(container.ID, "sed 's/pm.max_children = 5/pm.max_children = 6/g' /workspace/.php.fpm.d/user.conf > /tmp/sed_tmp && cat /tmp/sed_tmp > /workspace/.php.fpm.d/user.conf")
			Expect(err).NotTo(HaveOccurred())
