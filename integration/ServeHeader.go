...
// ServeHeader matches if the actual occam.Container is running AND the
// response from an HTTP request to the container's exposed port
// contains a header with the expected value
func ServeHeader(expectedName, expectedValue string) *ServeMatcher {
	return &ServeMatcher{
...
func (sm *ServeMatcher) Match(actual interface{}) (success bool, err error) {
	...
	if response != nil && response.StatusCode == http.StatusOK {
		sm.actualValue = response.Header.Get(sm.expectedName)
		match, err := sm.compare(sm.actualValue, sm.expectedValue)
		if err != nil {
			return false, err
		}
...
func (sm *ServeMatcher) FailureMessage(actual interface{}) (message string) {
	...
	message = fmt.Sprintf("Expected the response header %s from docker container %s:\n\n\t%s\n\nto contain:\n\n\t%s",
		sm.expectedName,
		container.ID,
		sm.actualValue,
		sm.expectedValue,
